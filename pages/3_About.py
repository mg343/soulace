...
st.write("**Economic Viability:** As a fine-tuned LLM, the only way users can prompt SoulAce and recieve responses quickly is by running the model on active servers. To make SoulAce free to users, inputs are reduced in a series of methods to decrease the amount of resources actively used. This reduces costs to levels that can are negligible for a majority of operations, so the app assumes the cost of the model in itself.")
...
st.write("**User Privacy:** To keep our users information personal, every instance of SoulAce is hosted independantly of others through the Streamlit Community Cloud. On top of this, no memory of conversations is every saved or utilized in any form. In fact, no information ever inputted by a user is saved to SoulAce. This design is rarely seen in chatbot infrastructure, further showing our commitment to the privacy of our users.")
